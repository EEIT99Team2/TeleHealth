...
public class GlobalService {
	...
	public static final String JNDI_DB_NAME = "java:comp/env/jdbc/TeleHealthDB";
	...
	// 等於maxLength。
	public static String adjustFileName(String photo, int maxLength) {
		if (photo != null) {
			int length = photo.length();
			if (length <= maxLength) {
				return photo;
			}
			int n = photo.lastIndexOf(".");
			int sub = photo.length() - n - 1;
			photo = photo.substring(0, maxLength - 1 - sub) + "." + photo.substring(n + 1);
			return photo;
		} else {
			return null;
		}
		...
		try {
			for (Part part : parts) {
				...
				if (contentType != null) { // 表示該part為檔案
					// 取出上傳檔案的檔名
					String photo = GlobalService.getFileName(part);
